...
def roll(attack_dice, defend_dice):
    ...
    attack_rolls = sorted([random.choice(FACES) for i in xrange(attack_dice)], reverse=True)
    defend_rolls = sorted([random.choice(FACES) for i in xrange(defend_dice)], reverse=True)
    if SHOW_ROLLS:
        print "Attacker rolls {}, defender rolls {}".format(attack_rolls, defend_rolls)
    attacker_deaths = 0
    defender_deaths = 0
    for i in xrange(len(defend_rolls)):
